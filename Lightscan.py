...
class Lightscan:
    ...
    def args_parse(self):
        ...
        self.parser.add_argument("-st","--scan_type",default="TCP", help="Scan types {TCP,SYN}")
    ...
    def udp_scan(self, port, target):
        for attempt in range(self.args.max_retries):
            try:
                ...
                self.scan_type = "udp"
                if port == 53:
                    packet = Payloads.dns_payload(target)
                else:
                    packet = scapy.IP(dst=target) / scapy.UDP(dport=port, sport=65535)
            ...
            try:
                ...
                if result == 0:
                    ...
                    if attempt == self.args.max_retries - 1:
                        ...
                        if self.args.verbose:
                            print(f"[!] Connection error on port {port} (error {result}), retrying... (attempt {attempt + 1}/{self.args.max_retries})")
                        s.close()
                        time.sleep(0.1)
                        continue
                ...
                if attempt == self.args.max_retries - 1:
                    ...
                    if self.args.verbose:
                        print(f"[!] Socket timeout on port {port}, retrying... (attempt {attempt + 1}/{self.args.max_retries})")
                    time.sleep(0.1)
                    continue

            except Exception as e:
                if self.args.verbose:
                    print(f"[!] Error scanning TCP connect port {port} (attempt {attempt + 1}/{self.args.max_retries}): {e}")
    ...
    def host_discovery(self, Target):
            Echo = scapy.IP(dst=Target) / scapy.ICMP()
